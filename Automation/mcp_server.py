...
RATE_LIMIT_WHITELIST = [c.strip() for c in os.environ.get('RATE_LIMIT_WHITELIST', '').split(',') if c.strip()]
...
class MCPHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == '/status':
            payload = {'ok': True, 'agents': list(self.server.agents.keys()), 'tasks': list(self.server.tasks), 'controllers': list(self.server.controllers.values())}
        ...
        if parsed.path == '/controllers':
            ...
            self._send_json({'ok': True, 'controllers': list(self.server.controllers.values())})
            return
        else:
    ...
    def do_POST(self):
        ...
        if parsed.path == '/run':
            ...
            if command not in ALLOWED_COMMANDS:
                self._send_json({'error': 'command_not_allowed', 'allowed': list(ALLOWED_COMMANDS.keys())}, status=403)
                return
            ...
            task = {'id': task_id, 'agent': agent, 'command': command, 'project': project, 'status': 'queued'}
            ...
            if execute:
                ...
                threading.Thread(target=self._execute_task, args=(task, cmd), daemon=True).start()
            return
        ...
        if parsed.path == '/workflow_alert':
            ...
            if not workflow or not conclusion:
                self._send_json({'error': 'workflow_and_conclusion_required'}, status=400)
                return
            ...
            task = {'id': task_id, 'agent': 'workflow-monitor', 'command': 'ci-check', 'project': head_branch or 'workspace', 'status': 'queued', 'meta': {'workflow': workflow, 'conclusion': conclusion, 'url': url, 'run_id': run_id, 'action': action}}
            self.server.tasks.append(task)
            # persist immediately
            try:
        ...
        if parsed.path == '/execute_task':
            ...
            try:
                with self.server.task_lock:
                    if target.get('status') != 'queued':
                        self._send_json({'error': 'task_not_queued', 'status': target.get('status')} , status=409)
                        return
            ...
            # spawn execution
            threading.Thread(target=self._execute_task, args=(target, cmd), daemon=True).start()
    ...
    def _execute_task(self, task, cmd):
        task['status'] = 'running'
        try:
            proc = subprocess.run(cmd, cwd=CODE_DIR, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=1800)
...
def run_server(host=HOST, port=PORT):
    ...
    cleanup_thread = threading.Thread(target=cleanup_loop, args=(stop_event,), daemon=True)
