...
def test_register_and_run_execute_flow():
    ...
    try:
        # register agent
        r = requests.post('http://127.0.0.1:5005/register', json={'agent': 'pytest-agent', 'capabilities': ['monitor']}, timeout=3)
        assert r.ok and r.json().get('registered') == 'pytest-agent'

        # enqueue a non-executable run (should be queued)
        r2 = requests.post('http://127.0.0.1:5005/run', json={'agent': 'pytest-agent', 'command': 'status', 'project': '', 'execute': False}, timeout=3)
        assert r2.ok and r2.json().get('queued')
        task_id = r2.json().get('task_id')

        # execute the task
        r3 = requests.post('http://127.0.0.1:5005/execute_task', json={'task_id': task_id}, timeout=3)
...
def test_modify_fail_and_rollback():
    ...
    try:
        ...
        r = requests.post('http://127.0.0.1:5005/register', json={'agent': 'pytest-exec', 'capabilities': ['execute']}, timeout=3)
        assert r.ok

        # enqueue modify-fail and request execute
        r2 = requests.post('http://127.0.0.1:5005/run', json={'agent': 'pytest-exec', 'command': 'modify-fail', 'project': '', 'execute': True}, timeout=3)
        assert r2.ok
            ...
            if os.path.isdir(tasks_dir):
                for fn in os.listdir(tasks_dir):
                    if fn.endswith('.json') and tid in fn:
                        ...
                        bakfiles = [x for x in os.listdir(os.path.dirname(target)) if (x.startswith(os.path.basename(target) + '.bak.') and '.bak.' in x)]
                        if bakfiles:
                            bak_found = True
                            break
            if bak_found:
