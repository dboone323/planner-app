...
class MenuBarManager: ObservableObject {
    ...
    private func setupQuickMenu() {
        ...
        menu.addItem(
            NSMenuItem(
                title: "Show Dashboard", action: #selector(self.showDashboard), keyEquivalent: ""
            )
        )
        menu.addItem(
            NSMenuItem(title: "Quit PlannerApp", action: #selector(self.quitApp), keyEquivalent: "q")
        )
...
extension TouchBarProvider: NSTouchBarDelegate {
    func touchBar(
        ...
        case .addTask:
            ...
            item.view = NSButton(title: "Add Task", target: self, action: #selector(self.addTask))
            (item.view as? NSButton)?.setAccessibilityLabel("Button")
            return item

        case .addGoal:
            let item = NSCustomTouchBarItem(identifier: identifier)
            item.view = NSButton(title: "Add Goal", target: self, action: #selector(self.addGoal))
            (item.view as? NSButton)?.setAccessibilityLabel("Button")
            return item
        ...
        case .calendar:
            ...
            item.view = NSButton(
                ...
                target: self, action: #selector(self.showCalendar)
            )
            (item.view as? NSButton)?.setAccessibilityLabel("Button")
            return item
