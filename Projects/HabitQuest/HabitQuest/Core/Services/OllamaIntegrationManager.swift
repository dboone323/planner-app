...
public class OllamaIntegrationManager: AITextGenerationService, AICodeAnalysisService, AICodeGenerationService {
    ...
    public func generateText(prompt: String, maxTokens: Int, temperature: Double) async throws -> String {
        let startTime = Date()
        let cacheKey = await cache.generateKey(for: prompt, model: "llama2", maxTokens: maxTokens, temperature: temperature)
    ...
    public func analyzeCode(code: String, language: String, analysisType: AnalysisType) async throws -> CodeAnalysisResult {
        let startTime = Date()
        let cacheKey = await cache.generateKey(for: code, model: "llama2", analysisType: analysisType.rawValue)
    ...
    public func generateTests(code: String, language: String) async throws -> String {
        let startTime = Date()
        let cacheKey = await cache.generateKey(for: code, model: "codellama", operation: "tests")
    ...
    public func generateCode(description: String, language: String, context: String?) async throws -> CodeGenerationResult {
        let startTime = Date()
        let cacheKey = await cache.generateKey(for: description, model: "codellama", context: context)
    ...
    private func processTask(_ task: AutomationTask) async throws -> TaskResult {
        ...
        case .documentation:
            ...
            return TaskResult(task: task, success: true, documentationResult: DocumentationResult(documentation: result, language: task.language ?? "Swift", includesExamples: true))
        ...
        case .testing:
            ...
            return TaskResult(task: task, success: true, testResult: TestGenerationResult(testCode: result, language: task.language ?? "Swift", testFramework: "XCTest", coverage: 0.8))
        }
    }
...
extension OllamaIntegrationManager {
    ...
    func checkServiceHealth() async -> OllamaHealthStatus {
        let serverStatus = await client.getServerStatus()
        let llama2Available = await client.checkModelAvailability("llama2")
        let codellamaAvailable = await client.checkModelAvailability("codellama")
        let availableModels = llama2Available && codellamaAvailable
...
// MARK: - Health monitoring for AI services
