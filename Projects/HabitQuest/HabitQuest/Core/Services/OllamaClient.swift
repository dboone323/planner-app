...
public class OllamaClient: ObservableObject {
    ...
    public func generate(
        ...
        if useCache, self.config.enableCaching {
            ...
            if let cached = await self.getCachedValue(for: cacheKey) {
        ...
        do {
            ...
            if useCache, self.config.enableCaching {
                let cacheKey = "\(requestModel):\(prompt.hashValue):\(requestTemp)"
                await self.setCachedValue(cacheKey, value: result)
            }

            // Record metrics
            let duration = Date().timeIntervalSince(startTime)
            await self.recordRequestMetrics(model: requestModel, duration: duration, tokenCount: result.count / 4)

            return result

        } catch {
            await self.recordErrorMetrics(error: error)
            throw error
        }
    }
    ...
    private func performRequest(endpoint: String, body: [String: Any]) async throws -> [String: Any] {
        guard let url = URL(string: "\(config.baseURL)/\(endpoint)") else {
            throw IntegrationError.invalidConfiguration("Invalid URL")
        }
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            throw IntegrationError.networkError("Invalid response type")
        }

        guard (200...299).contains(httpResponse.statusCode) else {
            throw IntegrationError.serverError("HTTP \(httpResponse.statusCode)")
        }

        guard let jsonResponse = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            throw IntegrationError.invalidResponseFormat("Expected JSON object")
        }
    ...
    private func recordRequestMetrics(model: String, duration: TimeInterval, tokenCount: Int) async {
        metrics.recordRequest(model: model, duration: duration, tokenCount: tokenCount)
    }

    private func recordErrorMetrics(error: Error) async {
