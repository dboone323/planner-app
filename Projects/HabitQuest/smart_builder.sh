...
test_ios() {
	...
	if CODE_SIGNING_ALLOWED=${CODE_SIGNING_ALLOWED:-NO} CODE_SIGNING_REQUIRED=${CODE_SIGNING_REQUIRED:-NO} xcodebuild test -scheme "${scheme}" -destination "platform=iOS Simulator,name=iPhone 16"; then
