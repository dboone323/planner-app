...
class PerformanceOptimizer:
    ...
    def get_optimization_suggestions(self) -> List[str]:
        ...
        if latest.cpu_percent > 80:
            suggestions.append(
                "High CPU usage detected - consider optimizing intensive processes"
            )

        if latest.memory_percent > 85:
            suggestions.append(
                "High memory usage detected - consider increasing swap or reducing memory-intensive tasks"
            )

        if latest.disk_usage > 90:
            suggestions.append(
                "High disk usage detected - consider cleanup or expanding storage"
            )
