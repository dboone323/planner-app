...
public class TransactionsDataGenerator: DataGenerator {
    ...
    /// Initializes the generator with a model context.
    /// - Parameter modelContext: The SwiftData model context.
    init(modelContext: ModelContext) {
        self.modelContext = modelContext
    }

    /// Generates and inserts sample transactions into the data store.
    func generate() {
        guard let categories = try? modelContext.fetch(FetchDescriptor<ExpenseCategory>()) else {
            return
        }
        guard let accounts = try? modelContext.fetch(FetchDescriptor<FinancialAccount>()) else {
            return
        }
...
public class SubscriptionsDataGenerator: DataGenerator {
    ...
    /// Generates and inserts sample subscriptions into the data store.
    func generate() {
        guard let categories = try? modelContext.fetch(FetchDescriptor<ExpenseCategory>()) else {
            return
        }
    ...
    /// - Parameters:
    ///   - checkingAccount: The user's checking account.
    ///   - creditCard: The user's credit card account.
    /// - Returns: An array of `SubscriptionData` for all categories.
    ...
    /// Creates sample entertainment subscriptions.
    ...
    /// Creates sample utility subscriptions.
    /// - Parameters:
    ///   - checkingAccount: The user's checking account.
    ///   - creditCard: The user's credit card account.
    /// - Returns: An array of utility `SubscriptionData`.
    private func createUtilitySubscriptions(
        checkingAccount: FinancialAccount, creditCard: FinancialAccount
    ) -> [SubscriptionData] {
        let calendar = Calendar.current
        let today = Date()
    ...
    /// Creates sample personal subscriptions.
    ...
    /// Creates sample transportation subscriptions.
    /// - Parameter checkingAccount: The user's checking account.
