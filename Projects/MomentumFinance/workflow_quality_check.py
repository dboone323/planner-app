...
class WorkflowQualityChecker:
    ...
    def check_code_quality(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.log_result(
                    "flake8_check", False, f"Style issues found: {result.stdout[:200]}"
                )
                return False
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.log_result(
                    "black_formatting", False, "Code formatting issues detected"
                )
                return False
        except (subprocess.TimeoutExpired, FileNotFoundError):
    ...
    def check_file_structure(self) -> bool:
        ...
        for file_path in required_files:
            if os.path.exists(file_path):
                self.log_result(
                    f"required_file_{file_path.replace('/', '_')}",
                    True,
                    f"Found {file_path}",
                )
            else:
        ...
        for file_path in recommended_files:
            if os.path.exists(file_path):
                self.log_result(
                    f"recommended_file_{file_path.replace('/', '_')}",
                    True,
                    f"Found {file_path}",
                )
            else:
    ...
    def check_python_syntax(self) -> bool:
        ...
        if syntax_errors:
            self.log_result(
                "python_syntax",
                False,
                f"Syntax errors found: {'; '.join(syntax_errors[:3])}",
            )
            return False
        else:
            self.log_result(
            ...
            try:
                ...
                if invalid_deps:
                    self.log_result(
                        "requirements_format",
                        False,
                        f"Invalid dependency format: {invalid_deps[:3]}",
                    )
                    return False
                else:
                    self.log_result(
                        "requirements_format",
                        True,
                        f"All {len(valid_deps)} dependencies properly formatted",
                    )
    ...
    def check_ai_learning_integration(self) -> bool:
        ...
        if found_ai_features:
            self.log_result(
                "ai_integration",
                True,
                f"AI learning features detected: {len(found_ai_features)} components",
            )
        else:
            self.log_result(
                "ai_integration", True, "No AI learning integration (optional)"
            )
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            "summary": {
                ...
                "success_rate": self.checks_passed
    ...
    def run_all_checks(self) -> bool:
        ...
        print(
            "   ✅ Cross-project learning: Available for HabitQuest & MomentumFinance"
        )
