...
public struct PlannerAgent: BaseAgent {
    ...
    public func execute(context: [String: any Sendable]) async throws -> AgentResult {
        let tasks = context["tasks"] as? [PlannerTask] ?? []
        ...
        let highPriority = tasks.filter { $0.priority == .high || $0.priority == .critical }
        let overdue = tasks.filter {
            if let dueDate = $0.dueDate, dueDate < Date() { return true }
            return false
        }
    ...
    private func performScheduleOptimization(
        ...
        let taskList = tasks.map { "- \($0.title) (Priority: \($0.priority.displayName))" }.joined(
            separator: "\n"
        )
